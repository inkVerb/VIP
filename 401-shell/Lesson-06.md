...
- boolean *- only `true` or `false`*
  ...
  - Do not confuse boolean `1` and `0` (non-Shell languages) with STDIN (`0`), STDOUT (`1`), and STDERR (`2`), nor with other `exit` codes. ***Boolean only allows TWO values, `exit`/error codes have many more!***
